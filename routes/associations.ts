...
import {
  ...
  computeTaskAssociationsForUser,
...
router.get('/tasks/:id/related', asyncRoute(async (req, res) => {
  ...
  const related = await getRelatedTasksForUser(user.id, req.params.id, 5);
  return res.json(related);
}));

router.post('/knowledge/analyze', asyncRoute(async (req, res) => {
  const user = requireAuthUser(req);
  const result = await computeTaskAssociationsForUser(user.id);
