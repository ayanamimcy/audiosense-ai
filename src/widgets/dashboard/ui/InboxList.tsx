...
// Scalar selection flags (`isSelected`, `isBatchSelected`) prevent other rows
// from re-rendering when the active selection moves to a different task.
...
export function InboxList({
  ...
  // Memoize sort — avoid re-sorting on every render. Also memoize the selected-id Set
  // for O(1) batch-selected lookups instead of .includes() per row.
  const sorted = useMemo(
    () => [...tasks].sort((a, b) => (sortAsc ? a.createdAt - b.createdAt : b.createdAt - a.createdAt)),
    [tasks, sortAsc],
  );
  const visible = useMemo(
    () => (showAll ? sorted : sorted.slice(0, PAGE_SIZE)),
    [sorted, showAll],
  );
          ...
          return (
            <div key={task.id}>
              ...
              <TaskRow
                ...
                isBatchSelected={selectedIdSet.has(task.id)}
