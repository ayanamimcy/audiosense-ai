...
import { Book, Check, ChevronDown, Edit2, Loader2, Sparkles, Tag, Trash2, Users, Waves } from 'lucide-react';
...
export function TaskHeader({
  ...
  // Media loading state — shown during initial fetch and during buffering stalls.
  // `isMediaLoading` covers the initial metadata/canplay wait for the current task.
  // `isBuffering` is a transient flag toggled by `onWaiting`/`onPlaying` during playback.
  ...
  React.useEffect(() => {
    ...
    // Reset media state when switching to a different task
  ...
  // Shared media event handlers — centralize loading/buffering state machine.
      ...
      return (
        <div className="relative w-full">
          ...
          {(isMediaLoading || isBuffering) && (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-slate-900/40 backdrop-blur-[1px]">
              <div className="flex items-center gap-2 rounded-full bg-slate-900/70 px-3 py-1.5 text-xs font-medium text-white shadow-lg">
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {isMediaLoading ? 'Loading video...' : 'Buffering...'}
              </div>
            </div>
          )}
        </div>
    ...
    return (
      <div className={cn(
        ...
        <audio
          ...
          onEnded={() => onTimeUpdate(-1)}
        />
        {(isMediaLoading || isBuffering) && !useBottomAudioBar && (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-start pl-4">
            <div className="flex items-center gap-1.5 rounded-md bg-white/90 px-2 py-0.5 text-[10px] font-medium text-slate-600 shadow-sm">
              <Loader2 className="h-3 w-3 animate-spin" />
              {isMediaLoading ? 'Loading...' : 'Buffering...'}
            </div>
          </div>
        )}
      </div>
