...
import { generateTaskSummary, isLlmConfigured } from '../ai/llm.js';
import logger from '../shared/logger.js';

const log = logger.child('post-processing');
...
const summaryStep: PostProcessingStep = {
  ...
  enabled(_task, settings) {
    const auto = settings?.autoGenerateSummary || config.transcription.autoGenerateSummary;
...
export async function runTaskPostProcessing(
  ...
  for (const step of steps) {
    ...
    if (!fresh || fresh.status !== 'completed') {
      log.info('Skipping remaining post-processing: status is no longer completed', { taskId });
      return;
    }
    if (runToken !== null && fresh.completedAt !== runToken) {
      log.info('Skipping remaining post-processing: completedAt changed (reprocessed)', { taskId });
      return;
    }
    currentTask = fresh;
    ...
    try {
      log.info('Starting post-processing step', { step: step.id, taskId });
      await step.run(currentTask, settings);
      log.info('Completed post-processing step', { step: step.id, taskId });
    } catch (error) {
      log.error('Failed post-processing step', { step: step.id, taskId, error: error instanceof Error ? error.message : String(error) });
    }
  }
}
