...
const log = logger.child('tag-suggestions');
import {
  findTaskRowById,
  listTaskTagRowsByUser,
  updateTaskRowById,
} from '../../database/repositories/tasks-repository.js';
import { generateTaskTagSuggestions } from '../ai/llm.js';
...
export async function persistTaskTagSuggestions(
  ...
  try {
    ...
    log.error('Failed to generate tag suggestions', { taskId, error: error instanceof Error ? error.message : String(error) });
    const latest = await findTaskRowById(taskId);
    if (!latest || getTaskTagSuggestionRequestId(latest) !== requestId) {
      return;
    }
