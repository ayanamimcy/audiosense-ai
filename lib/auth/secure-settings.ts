...
const log = logger.child('secure-settings');
...
function getDefaultSettingsKeyFilePath() {
  const baseDir = config.db.isSqlite
    ? path.dirname(config.db.sqliteFilename)
    : path.resolve(process.cwd(), 'runtime-data');

  return path.resolve(
    config.security.encryptionKeyFile ||
...
function readOrCreateSettingsKeyMaterial() {
  ...
  try {
    ...
    log.error('CRITICAL: Failed to load or create encryption key — API keys will be stored in plaintext', { error: error instanceof Error ? error.message : String(error) });
    return '';
  }
}
